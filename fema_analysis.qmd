...
# state_nri <- nri_data |> 
#     group_by(state_name) |> 
#     summarise(mean_nri_score = mean(national_risk_index_score_composite, na.rm = TRUE))
...
```{r}
#| fig-alt: "Boxplots comparing FEMA National Risk Index Scores (1-100) in California counties and other states, showing that California counties have higher NRI scores on average."
ggplot(nri_data, aes(x = national_risk_index_score_composite, y = fct_reorder(is_ca, national_risk_index_score_composite))) +
  geom_boxplot(fill = c("grey59", "skyblue")) +
  labs(
    x = "NRI Score",
    title = "FEMA National Risk Index (NRI) Comparison",
    subtitle = "California Counties vs Other States"
  ) +
  theme_minimal() +
